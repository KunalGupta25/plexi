...
export function PDFViewer({ url, filename = "Document", className, isMobile = false, shareData }: PDFViewerProps) {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(isMobile ? "scroll" : "single")
  ...
  const [undoStack, setUndoStack] = useState<Map<number, Annotation[][]>>(new Map())
  const [redoStack, setRedoStack] = useState<Map<number, Annotation[][]>>(new Map())
    ...
    const loadPDF = async () => {
      try {
        ...
        if (err?.message?.includes("CORS") || err?.message?.includes("fetch")) {
          setError("PDF blocked by browser security. Try opening in a new tab.")
        } else {
  ...
  const handlePointerMove = (e: React.MouseEvent | React.TouchEvent, pageNum: number = currentPage) => {
    if (!isDrawing || currentTool === "none" || currentTool === "text" || isMobile) return
  ...
  if (error) {
    return (
      <div className={cn("flex flex-col items-center justify-center h-full bg-muted/30 p-4", className)}>
      ...
      <div className="flex items-center justify-between gap-2 px-3 py-2 bg-card border-b border-border shrink-0">
        ...
        {isMobile && shareData && (
          <Button
            ...
            title="Share link"
          >
            {showCopied ? <Check className="h-4 w-4 text-green-500" /> : <Share2 className="h-4 w-4" />}
          </Button>
        )}
        ...
        {!isMobile && (
          <div className="flex items-center gap-1">
            <Button
              ...
              title={viewMode === "single" ? "Switch to vertical scroll" : "Switch to single page"}
            >
      ...
      {!isMobile && (
        <div className="flex items-center gap-1 px-3 py-2 bg-card border-b border-border shrink-0">
          ...
          <Button
            ...
            size="icon"
            className="h-8 w-8"
            onClick={() => setCurrentTool(currentTool === "highlighter" ? "none" : "highlighter")}
            title="Highlighter"
          >
